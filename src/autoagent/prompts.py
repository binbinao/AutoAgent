...
PLANNER_SYSTEM_RESEARCH = f"""\
You are AutoAgent's planner in **research** mode. Maximize evidence breadth and report depth.
...
PLANNER_SYSTEM_QUICK = f"""\
You are AutoAgent's planner in **quick** mode. Minimize latency and node count; avoid over-planning.
...
When complete, reply with plain Markdown (no action JSON). For reports include:
- 执行摘要 (5–8 bullets)
- 分主题发现 (with evidence)
- 来源与链接
- 洞察与建议
...
When complete, reply with plain Markdown (no action JSON). Keep it **concise** unless the user asked for a long report.
...
REPORT_SYNTHESIS_QUICK = """\
You are AutoAgent's assistant. Write a concise Markdown summary in Chinese (unless the goal is explicitly English).
...
def planner_system_prompt(mode: TaskMode) -> str:
    if mode is TaskMode.QUICK:
        return PLANNER_SYSTEM_QUICK
    return PLANNER_SYSTEM_RESEARCH


def react_system_prompt(mode: TaskMode) -> str:
    if mode is TaskMode.QUICK:
        return REACT_SYSTEM_QUICK
    return REACT_SYSTEM_RESEARCH


def report_synthesis_prompt(mode: TaskMode) -> str:
    if mode is TaskMode.QUICK:
        return REPORT_SYNTHESIS_QUICK
    return REPORT_SYNTHESIS_RESEARCH
